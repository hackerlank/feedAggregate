...
window['rangy'] = (function() {
    ...
    function init() {
        ...
        var testRange;
        var implementsDomRange = 0, implementsTextRange = 0;
    ...
    if (isHostMethod(document, "addEventListener")) {
        document.addEventListener("DOMContentLoaded", loadHandler, 0);
    }

    // Add a fallback in case the DOMContentLoaded event isn't supported
    if (isHostMethod(window, "addEventListener")) {
        window.addEventListener("load", loadHandler, 0);
...
rangy.createModule("DomUtil", function(api, module) {
    ...
    function iterateSubtree(rangeIterator, func, iteratorState) {
        var it, n;
        iteratorState = iteratorState || { stop: 0 };
    ...
    function getNodesInRange(range, nodeTypes, filter) {
        ...
        var nodes = [];
        iterateSubtree(new RangeIterator(range, 0), function(node) {
    ...
    RangePrototype.prototype = {
        ...
        containsNode: function(node, allowPartial) {
            if (allowPartial) {
                return this.intersectsNode(node, 0);
            } else {
    ...
    function createPrototypeRange(constructor, boundaryUpdater, detacher) {
        ...
        api.util.extend(constructor.prototype, {
            ...
            setStartAfter: createBeforeAfterNodeSetter(0, true),
            setEndBefore: createBeforeAfterNodeSetter(true, 0),
            setEndAfter: createBeforeAfterNodeSetter(0, 0),
            ...
            selectNode: function(node) {
                assertNotDetached(this);
                assertNoDocTypeNotationEntityAncestor(node, 0);
                assertValidNodeType(node, beforeAfterNodeTypes);
    ...
    - the parentElement() of the textRange after calling collapse(0)
     */
...
*/
        ...
        WrappedRange.prototype.refresh = function() {
            ...
            if (textRangeIsCollapsed(this.textRange)) {
                ...
                start = getTextRangeBoundaryPosition(this.textRange, rangeContainerElement, true, 0);
                end = getTextRangeBoundaryPosition(this.textRange, rangeContainerElement, 0, 0);
            }
    ...
    if (api.features.implementsTextRange) {
        WrappedRange.rangeToTextRange = function(range) {
            if (range.collapsed) {
                ...
                var endRange = createBoundaryTextRange(new DomPosition(range.endContainer, range.endOffset), 0);
    ...
    var selectionSupportsMultipleRanges = 0;
    ...
    // ControlRanges
    var implementsControlRange = 0, testControlRange;
    ...
    if (selectionHasAnchorAndFocus) {
        ...
        selectionIsCollapsed = function(sel) {
            return sel.rangeCount ? sel.getRangeAt(sel.rangeCount - 1).collapsed : 0;
        };
    }
    ...
    function updateFromTextRange(sel, range) {
        ...
        updateAnchorAndFocusFromRange(sel, wrappedRange, 0);
        sel.rangeCount = 1;
    ...
    function updateControlSelection(sel) {
        ...
        if (sel.docSelection.type == "None") {
            ...
            if (isTextRange(controlRange)) {
                ...
                updateAnchorAndFocusFromRange(sel, sel._ranges[sel.rangeCount - 1], 0);
            }
        }
    }
    ...
    if (!useDocumentSelection && selectionHasAnchorAndFocus && util.areHostMethods(testSelection, ["removeAllRanges", "addRange"])) {
        ...
        selProto.addRange = function(range) {
            if (this.docSelection.type == CONTROL) {
                ...
                updateAnchorAndFocusFromRange(this, range, 0);
            }
        };
    ...
    var removeRangeManually = function(sel, range) {
        var ranges = sel.getAllRanges(), removed = 0;
        sel.removeAllRanges();
        for (var i = 0, len = ranges.length; i < len; ++i) {
...
wysihtml5.browser = (function() {
  ...
  return {
    ...
    autoClosesUnclosedTags: function() {
      var clonedTestElement = testElement.cloneNode(0),
          returnValue,
          innerHTML;
...
wysihtml5.dom.observe = function(element, eventNames, handler) {
  ...
  for (; i<length; i++) {
    ...
    if (element.addEventListener) {
      element.addEventListener(eventName, handler, 0);
    } else {
    ...
    stop: function() {
      ...
      for (; i<length; i++) {
        ...
        if (element.removeEventListener) {
          element.removeEventListener(eventName, handler, 0);
        } else {
...
(function(wysihtml5) {
  ...
  wysihtml5.dom.Sandbox = Base.extend(
     ...
     *  - Believe it or not but in IE "security" in document.createElement("iframe") is 0, even
...
(function(wysihtml5, rangy) {
  ...
  function splitNodeAt(node, descendantNode, descendantOffset) {
    ...
    if (!newNode) {
      newNode = descendantNode.cloneNode(0);
      if (newNode.id) {
  ...
  HTMLApplier.prototype = {
    ...
    postApply: function(textNodes, range) {
      ...
      for (var i = 0, len = textNodes.length; i < len; ++i) {
        ...
        precedingTextNode = this.getAdjacentMergeableTextNode(textNode.parentNode, 0);
...
wysihtml5.views.View = Base.extend(
  ...
  wysihtml5.views.Composer = wysihtml5.views.View.extend(
    ...
    _create: function() {
      ...
      wysihtml5.commands.exec(this.element, "styleWithCSS", 0);
    ...
    _initAutoLinking: function() {
      ...
      if (supportsDisablingOfAutoLinking) {
        wysihtml5.commands.exec(this.element, "autoUrlDetect", 0);
      }
  ...
  wysihtml5.views.Composer.prototype.style = function() {
    ...
    this.focusStylesHost      = this.focusStylesHost  || HOST_TEMPLATE.cloneNode(0);
    this.blurStylesHost       = this.blurStylesHost   || HOST_TEMPLATE.cloneNode(0);
...
(function(wysihtml5) {
  ...
  wysihtml5.toolbar.Toolbar = Base.extend(
    ...
    _execCommand: function(command, commandValue) {
      // Make sure that composer is focussed (0 => don't move caret to the end)
      this.editor.focus(0);
  ...
  exec: function(element, command, value) {
    ...
    if (method) {
      ...
      try {
        ...
        return element.ownerDocument.execCommand(command, 0, value);
      } catch(e) {}
    }
  },
...
(function(wysihtml5) {
  ...
  function _execCommand(doc, command, nodeName, className) {
    ...
    doc.execCommand(command, 0, nodeName);
    if (eventListener) {
      eventListener.stop();
    }
  }
...
(function(wysihtml5) {
  ...
  wysihtml5.commands.insertHTML = {
    exec: function(element, command, html) {
      if (wysihtml5.commands.support(element, command)) {
        element.ownerDocument.execCommand(command, 0, html);
      } else {
  ...
  wysihtml5.commands.insertLineBreak = {
    exec: function(element, command) {
      if (wysihtml5.commands.support(element, command)) {
        element.ownerDocument.execCommand(command, 0, null);
  ...
  wysihtml5.commands.insertOrderedList = {
    exec: function(element, command) {
      ...
      if (wysihtml5.commands.support(element, command)) {
        doc.execCommand(command, 0, null);
      } else {
        selectedNode = wysihtml5.selection.getSelectedNode(doc);
    ...
    _observe: function() {
      ...
      dom.observe(this.composerElement, "contextmenu", function() {
        ...
        wysihtml5.selection.executeAndRestoreSimple(doc, function() {
          ...
          doc.execCommand("insertHTML", 0, UNDO_HTML);
          doc.execCommand("insertHTML", 0, REDO_HTML);
          doc.execCommand("undo", 0, null);
        });
...
(function(wysihtml5) {
  ...
  var defaultConfig = {
    ...
    style:                true,
    // Id of the toolbar element, pass 0y value if you don't want any toolbar logic
    toolbar:              undef,
