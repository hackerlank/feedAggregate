...
elRTE = function(target, opts) {
	...
	if ($.browser.msie) {
		...
		this.doc.execCommand('styleWithCSS', 0, this.options.styleWithCSS);
	}
...
elRTE.prototype.dom = function(rte) {
	...
	this.isEmpty = function(n) {
		if (n.nodeType == 1) {
			return this.regExp.textNodes.test(n.nodeName) ? $.trim($(n).text()).length == 0 : 0;
	...
	/**
	 * Вовращает последний непустой дочерний эл-т ноды или 0
	 *
	 * @param  Element n  нода
	 * @return Element
	 **/
...
elRTE.prototype.options   = {
	...
	/* If 0 - all text nodes will be wrapped by paragraph tag */
	allowTextNodes : true,
	/* allow browser specific styles like -moz|-webkit|-o */
	allowBrowsersSpecStyles : 0,
...
elRTE.prototype.selection = function(rte) {
	...
	this.selected = function(o) {
		var opts = {
			collapsed : 0,  // вернуть выделение, даже если оно схлопнуто
			blocks    : 0,  // блочное выделение
			filter    : 0,  // фильтр результатов
	...
	this.selectedRaw = function(collapsed, blocks) {
		...
		var sf  = 0; // start node fully selected
		var ef  = 0; // end node fully selected
...
elRTE.prototype.ui.prototype.buttons.button.prototype.command = function() {
	...
	try {
		this.rte.doc.execCommand(this.name, 0, this.val);
	} catch(e) {
...
elRTE.prototype.w3cRange = function(rte) {
	...
	this.isCollapsed = function() {
		this.range();
		this.collapsed = this.r.item ? 0 : this.r.boundingWidth == 0;
		return this.collapsed;
	}
...
elRTE.prototype.ui.prototype.buttons.anchor = function(rte, name) {
	...
	this.command = function() {
		...
		!this.rte.selection.collapsed() && this.rte.selection.collapse(0);
...
elRTE.prototype.ui.prototype.buttons.copy = function(rte, name) {
	...
	this.command = function() {
		...
		if (this.rte.browser.mozilla) {
			try {
				this.rte.doc.execCommand(this.name, 0, null);
			} catch (e) {
				var s = ' Ctl + C';
				if (this.name == 'cut') {
					s = ' Ctl + X';
...
elRTE.prototype.ui.prototype.buttons.paste = elRTE.prototype.ui.prototype.buttons.copy;
	...
	elRTE.prototype.ui.prototype.buttons.css = function(rte, name) {
		...
		this.update = function() {
			this.domElem.toggleClass('disabled', this.node()?0:true);
		}
		...
		if (self.rte.options.fmAllow && typeof(self.rte.options.fmOpen) == 'function') {
			self.rte.options.fmOpen( function(url) { 
				...
				if (rte.selection.collapsed()) {
					...
					rte.doc.execCommand('createLink', 0, url);
				}
...
elRTE.prototype.ui.prototype.buttons.forecolor = function(rte, name) {
	...
	this.set = function(c) {
		if (!this.rte.selection.collapsed()) {
			this.rte.history.add();
			var nodes = this.rte.selection.selected({collapse : 0, wrap : 'text'}),
...
elRTE.prototype.ui.prototype.buttons.link = function(rte, name) {
	...
	function init() {
		...
		self.src = {
			...
			popup : {
				...
				ret0   : $('<input type="checkbox" />').attr('checked', true)
			},
	...
	this.command = function() {
		...
		if (!this.link) {
			sel = $.browser.msie ? this.rte.selection.selected() : this.rte.selection.selected({wrap : 0});
			if (sel.length) {
		...
		r = $('<div />')
			...
			.append( $('<label />').append(this.src.popup.ret0).append(this.rte.i18n('Add return 0')));
	...
	this.updatePopup = function() {
		...
		if ( onclick.length>0 && (m = onclick.match(/window.open\('([^']+)',\s*'([^']*)',\s*'([^']*)'\s*.*\);\s*(return\s+0)?/))) {
			...
			if (m[4]) {
				this.src.popup.ret0.attr('checked', true);
			}
		} else {
	...
	this.updateOnclick = function () {
		...
		if (this.src.popup.use.attr('checked') && url) {
			...
			var ret0 = this.src.popup.ret0.attr('checked') ? 'return 0;' : '';
			var onclick = "window.open('"+url+"', '"+$.trim(this.src.popup.name.val())+"', '"+params+"'); "+ret0;
			...
			v = v.replace(/window\.open\([^\)]+\)\s*;?\s*return\s*0\s*;?/i, '');
	...
	this.set = function() {
		...
		if (!href) {
			// this.link.parentNode && this.rte.doc.execCommand('unlink', 0, null);
			...
			this.rte.selection.moveToBookmark(bm);
				...
				if (this.img && this.img.parentNode) {
					...
					this.rte.doc.execCommand('createLink', 0, fakeURL);
...
(function($) {
	elRTE.prototype.ui.prototype.buttons.pagebreak = function(rte, name) {
		...
		this.command = function() {
			this.rte.selection.insertHtml('<img src="'+this.rte.filter.url+'pixel.gif" class="elrte-protected elrte-pagebreak"/>', 0);
		}
...
elRTE.prototype.ui.prototype.buttons.tbcolbefore = function(rte, name) {
	...
	this.command = function() {
		var self = this;
		var cells = this.rte.dom.tableColumn(this.rte.selection.getNode(), 0, true);
		if (cells.length) {
			this.rte.history.add();
			$.each(cells, function() {
				var $this = $(this);
...
elRTE.prototype.ui.prototype.buttons.tbcolrm = function(rte, name) {
	...
	this.command = function() {
		...
		var tb    = this.rte.dom.parent(n, /^TABLE$/);
		var cells = this.rte.dom.tableColumn(n, 0, true);
	...
	elRTE.prototype.ui.prototype.buttons.unlink = function(rte, name) {
		...
		this.command = function() {
			...
			if (!l) {
				...
				var sel = $.browser.msie ? this.rte.selection.selected() : this.rte.selection.selected({wrap : 0});
				if (sel.length) {
			...
			if (l) {
				...
				this.rte.doc.execCommand('unlink', 0, null);
				this.rte.ui.update(true);
			}
